...
/// Notifier that manages the plumbing jobs state.
class JobsNotifier extends Notifier<List<PlumbingJob>> {
  ...
  List<PlumbingJob> build() {
    return [
      PlumbingJob(
        ...
        issues: ['Tempering valve tested at 48.5°C (Compliant)'],
      ),
      PlumbingJob(
        id: '2',
        ...
        issues: ['Missing as-constructed drainage plan upload', 'DN100 pipe gradient under 1.65%'],
      ),
      PlumbingJob(
        id: '3',
  ...
  /// Adds a new plumbing job to the list.
  void addJob(PlumbingJob job) {
    state = [...state, job];
  }

  /// Toggles the Form 4 lodgement status of a job.
  void lodgeForm4(String id) {
    state = [
      for (final job in state)
        if (job.id == id)
          job.copyWith(status: 'Lodged', form4Submitted: true, complianceScore: 1.0)
        else
          job
    ];
  }
